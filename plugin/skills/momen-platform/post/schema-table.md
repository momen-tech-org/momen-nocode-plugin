...
```bash
npx -y momen-mcp@2.7.1 schema tool-call --toolCalls '[{"name":"GET_TABLE_FIELD_SELECTABLE_TYPES","args":{}}]'
...
- If results look stale, run `npx -y momen-mcp@2.7.1 schema reload`.
...
npx -y momen-mcp@2.7.1 runtime insert --args '{"tableName":"post","objects":[{"title":"hi"}],"fields":["id"]}'
npx -y momen-mcp@2.7.1 runtime update --args '{"tableName":"post","where":{"id":{"_eq":1}},"set":{"title":"bye"}}'
npx -y momen-mcp@2.7.1 runtime delete --args '{"tableName":"post","where":{"id":{"_eq":1}}}'
```
