...
```bash
npx -y momen-mcp@2.6.1 secret save-value --displayName "OpenAI API key"   # create a secret and set its value
npx -y momen-mcp@2.6.1 secret save-value --secretId <id>                  # set / replace an existing secret's value
```
