...
```bash
npx -y momen-mcp@2.7.8 projects search --projectName "My App"
npx -y momen-mcp@2.7.8 project set-current --projectExId <exId>
npx -y momen-mcp@2.7.8 schema load
```
...
> **Always invoke the CLI exactly as written above** — via `npx -y momen-mcp@2.7.8` — never bare `momen-mcp`, even though `--help` prints its own name that way. The `@2.7.8` version pin runs this plugin's exact published build, so a globally-installed `momen-mcp` (or `momen`) on `PATH` can't shadow it.
