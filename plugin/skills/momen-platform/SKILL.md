...
2. **Data**: The persistence layer, powered by a built-in, highly-scalable relational database engine (PostgreSQL). All UI contents are dynamically populated by binding to database rows or API schemas.
...
- **Third-party API configs (TPA)** (`third-party-api.md`) — Inspect and edit Third-Party API (TPA) configs: saved external REST endpoints the app calls from a "Call API" action-flow node or from Run Code. Use to integrate or import an external API — from its online docs or an OpenAPI/Swagger spec — by registering an endpoint (url, method, and query/path/header/body parameters), describing the JSON it returns per success/failure branch, or setting list pagination. Works on every project generation.
...
**Determine the project's type-system generation FIRST.** `schema load` / `schema status` report `typeSystem`: `pre_type_system_refactor` or `post_type_system_refactor`. Read it before choosing operations — post-only capabilities (enums, object/custom types, API-integration workspaces) are hard-gated and do not exist on pre-refactor projects; TPA configs (`third-party-api.md`) work on both generations. Never set `MCP_TYPE_SYSTEM=post` to get past a gate: it cannot migrate a project, and post-only config written into a pre-refactor schema is invisible to the editor. The daemon auto-detects genuinely migrated projects.
