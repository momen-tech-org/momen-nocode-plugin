...
```bash
npx -y momen-mcp@2.6.0 projects search --projectName "My App"
npx -y momen-mcp@2.6.0 project set-current --projectExId <exId>
npx -y momen-mcp@2.6.0 schema load
```
...
> **Always invoke the CLI exactly as written above** — via `npx -y momen-mcp@2.6.0` — never bare `momen-mcp`, even though `--help` prints its own name that way. The `@2.6.0` version pin runs this plugin's exact published build, so a globally-installed `momen-mcp` (or `momen`) on `PATH` can't shadow it.
