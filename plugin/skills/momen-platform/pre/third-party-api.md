...
```bash
npx -y momen-mcp@2.7.0 tpa fetch-doc --url "https://api.example.com/docs"          # read a docs page: Markdown content, same-site pageLinks, OpenAPI specLinks; --offset to keep reading
npx -y momen-mcp@2.7.0 tpa import-from-openapi --url "https://api.example.com/openapi.json" --pathsFilter /users /orders   # spec URL (or --specText '<json|yaml>'); pathsFilter values are space-separated substrings
```
...
`secrets.md` for the ops and for `npx -y momen-mcp@2.7.0 secret save-value`, which collects the plaintext from the
